...
const std::string Analyzer::isMaliciousTiming(const std::list<Record> &records) const
{
    ...
    {
        LOG(WARNING) << "Analyzer isMaliciousTiming() faild to analyze records";
        LOG(WARNING) << e.what();
        // throw MyException("");
    }
    return "";
...
std::map<std::string, std::string> Analyzer::analyze(const std::list<Record> &records)
{
    ...
    try
    {
        ...
        if (!tmp.empty())
            res.insert_or_assign("TIMING_ALERT", tmp);
    }
    catch (const std::exception &e)
    {
        LOG(WARNING) << "Faild to analyze records";
        LOG(WARNING) << e.what();
        // throw MyException("");
    }
    return res;
...
void Analyzer::load(const std::string &ofp)
{
    ...
    for (auto it = root.begin(); it != root.end(); it++)
    {
        ...
        {
            LOG(WARNING) << "Faild while reading file";
            LOG(WARNING) << e.what();
            // throw MyException("");
        }
    }
...
void Analyzer::add(const std::string &process, const std::list<Record> &records)
{
    ...
    {
        LOG(WARNING) << "Failed to add record for process: " << process;
        LOG(WARNING) << e.what();
    }
}
...
void Analyzer::toFile() const
{
    ...
    {
        LOG(ERROR) << "Failed to write output file: " << outputFilePath;
        LOG(ERROR) << e.what();
        throw MyException("CREATE_FILE_FAILED Analyzer Failed to open output file");
    }
}
